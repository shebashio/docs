...
## ezRMF MCP integration

[ezRMF](/rmf/index) exposes a Model Context Protocol (MCP) server with 30+ compliance tools that enable AI-powered compliance assistance. The MCP server provides tools for analyzing evidence gaps, mapping controls, suggesting implementations, generating documents, and exporting audit packages — all within the context of your ezRMF project data.

You can connect Claude Desktop, Claude Code, or other MCP-compatible clients to the ezRMF MCP server to interact with your compliance data conversationally. See [MCP integration](/rmf/mcp-integration) for setup instructions.
...
<CardGroup cols={2}>
  ...
  <Card title="ezRMF MCP integration" icon="scale-balanced" href="/rmf/mcp-integration">
