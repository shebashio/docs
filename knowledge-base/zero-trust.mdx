...
sidebarTitle: "Zero trust access"
---
...
- **kasm.dedzed.blacklabel.mil** — AVA-protected endpoint for authentication and API calls
- **kasm-direct.dedzed.blacklabel.mil** — WAF-protected endpoint for WebSocket desktop connections (bypasses AVA)
...
flowchart TB
    ...
    subgraph Cloudflare["Cloudflare DNS"]
        DNS1["kasm.dedzed.blacklabel.mil"]
        DNS2["kasm-direct.dedzed.blacklabel.mil"]
    end
...
sequenceDiagram
    ...
    B->>CF: kasm.dedzed.blacklabel.mil
    CF->>AVA: HTTPS
    AVA->>B: PingFederate Login
    B->>AVA: Authenticate
    AVA->>WA: Authorized Request
    WA->>B: Set cookies (domain=.dedzed.blacklabel.mil)
    ...
    B->>CF: kasm-direct.dedzed.blacklabel.mil/desktop/{uuid}/...
...
|--------|------|---------|-------------------|
| `kasm.dedzed.blacklabel.mil` | AWS Verified Access | Authentication, API, session management | None (full access) |
| `kasm-direct.dedzed.blacklabel.mil` | WAF + ALB | WebSocket desktop connections | `/desktop/{uuid}` (enforced at WAF + ALB) |
...
|-------|-----------|-------------|-------------------|
| 1 | **AWS WAF** | Origin header = `https://kasm.dedzed.blacklabel.mil` | Yes (curl/scripts can set any Origin) |
...
|----------|------|------|--------|---------|
| 1 | Origin header validation | Custom | Block | Only allow `Origin: https://kasm.dedzed.blacklabel.mil` |
...
Blocks any request where the `Origin` header is not exactly `https://kasm.dedzed.blacklabel.mil`. In browsers, Origin is a forbidden header (cannot be spoofed by JavaScript), which eliminates all browser-based attacks including direct navigation, cross-origin scripts, and CSRF.
...
| Setting | Value |
|---------|-------|
| Certificate | `*.dedzed.blacklabel.mil` (ACM) |
...
|---------|-------|---------|
| Kasm Auth Domain | `.dedzed.blacklabel.mil` | Share cookies across subdomains |

### Zone settings

| Setting | Value | Purpose |
|---------|-------|---------|
| Upstream Auth Address | `kasm.dedzed.blacklabel.mil` | Agent validates sessions via webapp |
| Proxy Hostname | `kasm-direct.dedzed.blacklabel.mil` | WebSocket connection hostname |
...
| Record | Type | Target |
|--------|------|--------|
| `kasm.dedzed.blacklabel.mil` | — | AWS Verified Access endpoint |
| `kasm-direct.dedzed.blacklabel.mil` | CNAME | Application Load Balancer |
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| 403 from WAF | Origin header missing or incorrect | Verify browser sends `Origin: https://kasm.dedzed.blacklabel.mil` |
...
| No cookies sent to kasm-direct | Wrong cookie domain | Set Kasm Auth Domain to `.dedzed.blacklabel.mil` |
| Agent internal_auth fails | Wrong Upstream Auth Address | Set to `kasm.dedzed.blacklabel.mil` (not kasm-direct) |
