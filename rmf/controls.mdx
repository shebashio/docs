---
title: "Controls"
description: "Work with the NIST 800-53 Rev 5 control catalog, track implementation status, and map controls to CCIs"
sidebarTitle: "Controls"
---

## Overview

Controls are the core building blocks of an ATO package. RMF includes the complete NIST 800-53 Rev 5 catalog and automatically selects the applicable controls when you create a project based on its impact level. You track the implementation status of each control, document how it is satisfied, and link evidence to demonstrate compliance.

## Control catalog

RMF provides the full NIST 800-53 Rev 5 catalog organized by control family. When you create a project and select an impact level (Low, Moderate, or High), RMF populates the project with the corresponding control baseline.

### Browse controls

Navigate to the **Controls** tab within a project to see all selected controls. You can:
...
<Info>
Implementation statements should describe the specific mechanisms, configurations, or processes that satisfy the control. For example, an implementation statement for AC-2 might describe the account provisioning process, approval workflows, and automated account reviews.
</Info>

### Control enhancements

Many controls have enhancements (e.g., `AC-2(1)`, `AC-2(2)`) that provide additional specificity. RMF tracks enhancements as separate items with their own implementation status and evidence links. Enhancements appear nested under their parent control in the controls list.
...
<Tip>
When you import CKL files from [STIGMATE](/stigmate/index), RMF automatically maps the scan results to the corresponding CCIs and controls. This gives you instant visibility into which controls have technical validation evidence.
</Tip>

### CCI coverage report

The CCI coverage report shows how many CCIs across your project have associated STIG results or evidence. Navigate to **Reports** > **CCI Coverage** to see:
...
For projects with hundreds of controls, RMF provides bulk operations to update multiple controls at once.
...
RMF supports importing control implementation data from external sources:

```bash
curl -X POST https://rmf.icbm.dev/api/v1/projects/{id}/controls/import \
...
During the assessment phase, the Security Control Assessor (SCA) reviews each control to determine whether it is implemented correctly and operating as intended.
...
The SCA records findings and recommendations for each control. Controls marked as **Other Than Satisfied** generate Plan of Action and Milestones (POA&M) items that track remediation.
...
<CardGroup cols={2}>
  ...
  <Card title="MCP integration" icon="robot" href="/rmf/mcp-integration">
    Use AI to analyze evidence gaps and suggest implementations.
  </Card>
</CardGroup>
